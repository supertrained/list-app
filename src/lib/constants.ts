export const APP_NAME = 'L.I.S.T.';
export const APP_FULL_NAME = 'Life in Simple Terms';
export const APP_TAGLINE = 'Clear answers to your government and life admin questions';

export const EXAMPLE_QUESTIONS = [
  {
    text: 'How do I link my Medicare to MyGov?',
    category: 'MyGov',
  },
  {
    text: 'What documents do I need to apply for the Age Pension?',
    category: 'Pensions',
  },
  {
    text: 'How do I update my address with Centrelink?',
    category: 'Centrelink',
  },
  {
    text: 'What is a Home Care Package and how do I apply?',
    category: 'Aged Care',
  },
  {
    text: 'How do I lodge my tax return with the ATO?',
    category: 'Tax',
  },
  {
    text: 'How do I get a Seniors Card in NSW?',
    category: 'NSW Services',
  },
];

export const LOADING_MESSAGES = [
  'Looking up the latest information...',
  'Finding the clearest answer for you...',
  'Checking official sources...',
  'Almost there...',
];

export const MAX_QUESTION_LENGTH = 500;
export const MAX_PROMPT_LENGTH = 2000;

export const DISCLAIMER_STORAGE_KEY = 'list-disclaimer-accepted';
